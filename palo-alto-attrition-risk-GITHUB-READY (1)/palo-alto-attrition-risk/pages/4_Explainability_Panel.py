...
with col_rail:
    ...
    imp_chart_type = st.radio("Importance chart", ["Bar", "Treemap"], key="imp_chart_type")
    top_n_features = st.slider("Top N features", 5, 20, 15, key="imp_top_n")
    rail_end()

with col_main:
    st.markdown(f"#### Global Feature Importance — {best_name}")
    imp = global_feature_importance(pipeline, top_n=top_n_features)
    if imp_chart_type == "Bar":
        fig = go.Figure(go.Bar(
            x=imp["importance_pct"], y=imp["feature"], orientation="h", marker_color=COLORS["orange"],
        ))
        fig.update_layout(xaxis_title="Relative importance (%)", yaxis=dict(autorange="reversed"))
    else:
        fig = go.Figure(go.Treemap(
            labels=imp["feature"], parents=[""] * len(imp), values=imp["importance_pct"],
            marker=dict(colors=imp["importance_pct"], colorscale=[[0, COLORS["blue_accent"]], [1, COLORS["orange"]]]),
        ))
