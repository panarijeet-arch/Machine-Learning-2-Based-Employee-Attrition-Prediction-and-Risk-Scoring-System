...
a right-rail chart-type control, risk distribution, department
...
from theme import (
    COLORS, RISK_COLOR_MAP, hero, inject_global_css, insight_html,
    rail_start, rail_end, style_fig, short_model_name,
)
...
hero(
    ...
    "Predictive workforce analytics — built on real employee records, an interpretable model, "
    "and transparent per-employee explanations.",
...
c3.metric("Best Model", short_model_name(best_name))
c4.metric("Model ROC-AUC", f"{best_metrics['roc_auc']:.3f}")
c5.metric("Currently High-Risk", f"{high_risk_now}")

st.divider()

# --- Main content (left) + control rail (right) ---
col_main, col_rail = st.columns([3, 1])

with col_rail:
    rail_start("Chart Controls")
    risk_view = st.radio("Risk distribution view", ["Donut", "Bar"], key="home_risk_view")
    model_view = st.radio("Model comparison metric", ["ROC-AUC", "F1", "Recall"], key="home_model_metric")
    rail_end()
    ...
    with ch1:
        ...
        if risk_view == "Donut":
            ...
            fig = go.Figure(go.Bar(
                ...
                y=[low_risk_now, medium_risk_now, high_risk_now],
                marker_color=[RISK_COLOR_MAP["Low Risk"], RISK_COLOR_MAP["Medium Risk"], RISK_COLOR_MAP["High Risk"]],
                text=[low_risk_now, medium_risk_now, high_risk_now], textposition="outside",
            ))
        fig.update_layout(height=320, margin=dict(l=10, r=10, t=10, b=10), showlegend=(risk_view == "Donut"))
        st.plotly_chart(style_fig(fig), width="stretch")

    with ch2:
        st.markdown(f"#### Model Comparison ({model_view})")
        metric_key = {"ROC-AUC": "roc_auc", "F1": "f1", "Recall": "recall"}[model_view]
        comp_rows = [{"Model": n, "value": r["test_metrics"][metric_key]} for n, r in train_out["results"].items()]
        comp_df = pd.DataFrame(comp_rows).sort_values("value", ascending=False)
        fig2 = go.Figure(go.Bar(
            x=comp_df["Model"], y=comp_df["value"],
            marker_color=[COLORS["orange"] if m == best_name else COLORS["blue_accent"] for m in comp_df["Model"]],
            text=[f"{v:.3f}" for v in comp_df["value"]], textposition="outside",
        ))
        fig2.update_layout(height=320, margin=dict(l=10, r=10, t=10, b=10), yaxis_title=model_view)
