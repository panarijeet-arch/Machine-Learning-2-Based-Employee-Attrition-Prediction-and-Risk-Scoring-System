...
# st.metric's value text truncates with an ellipsis rather than wrapping,
# so a full model name like "Logistic Regression" gets cut to
# "Logistic..." inside a KPI card. Short display names avoid that.
SHORT_MODEL_NAMES = {
...
def brand_header() -> None:
    ...
    Uses st.columns for the icon+text layout rather than a custom CSS
    flexbox row -- an earlier flexbox version clipped the wordmark text
    (only letter-tops visible), a real layout bug. Streamlit's own
    column primitive is already proven reliable elsewhere in this app
    (every KPI row), so this reduces custom-layout surface area instead
    of debugging the flexbox further."""
    c1, c2 = st.columns([1, 10])
    with c1:
        if os.path.exists(LOGO_PATH):
            st.image(LOGO_PATH, width=40)
        else:
            st.markdown('<div class="nc-brand-mark">P</div>', unsafe_allow_html=True)
    with c2:
        st.markdown(
